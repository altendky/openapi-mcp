...
SPEC = importlib.util.spec_from_file_location("release_github", ROOT / ".github/scripts/release-github.py")
...
class TagTests(unittest.TestCase):
    def setUp(self):
        ...
        self.environ = {"GITHUB_REF": "refs/heads/main", "GITHUB_SHA": "main"}
        ...
        (self.root / "Cargo.toml").read_bytes.return_value = manifest("1.2.3")
        self.addCleanup(patch.stopall)

    def test_creates_only_validated_main_tag(self):
        self.assertEqual(release.tag_release(self.github, self.environ), (True, "1.2.3"))
    ...
    def test_skips_prerelease(self):
        (self.root / "Cargo.toml").read_bytes.return_value = manifest("1.2.4-dev.0")
        self.assertEqual(release.tag_release(self.github, self.environ), (False, "1.2.4-dev.0"))
        self.github.create_ref.assert_not_called()

    def test_rejects_build_metadata(self):
        (self.root / "Cargo.toml").read_bytes.return_value = manifest("1.2.3+build")
        with self.assertRaises(release.ReleaseError):
            release.tag_release(self.github, self.environ)
        self.github.create_ref.assert_not_called()
    ...
    def test_existing_tag_at_same_commit_is_idempotent(self):
        ...
        self.assertEqual(release.tag_release(self.github, self.environ), (False, "1.2.3"))
    ...
    def test_existing_same_version_tag_on_main_ancestor_is_idempotent(self):
        ...
        self.github.contents.assert_called_once_with("Cargo.toml", "old-main")
        self.github.create_ref.assert_not_called()
    ...
    def test_existing_ancestor_tag_with_different_version_is_rejected(self):
        ...
        with self.assertRaisesRegex(release.ReleaseError, "different workspace version"):
            release.tag_release(self.github, self.environ)
        self.github.create_ref.assert_not_called()
...
class PostReleaseTests(unittest.TestCase):
    def setUp(self):
        ...
        self.github.ref.side_effect = self.refs.get
        self.release_record = {"tag_name": "v1.2.3", "draft": False, "prerelease": False, "published_at": "2026-09-13"}
        self.github.api.return_value = self.release_record
        self.github.contents.return_value = manifest("1.2.3")
        self.environ = {"GITHUB_REF": "refs/tags/v1.2.3", "GITHUB_SHA": "tag-commit"}
        self.clean = patch.object(release, "clean_checkout").start()
        self.body = patch.object(release, "pull_request_body", return_value="Body").start()
        self.changes = patch.object(release, "local_changes", return_value={"Cargo.toml": manifest("1.2.4-dev.0")}).start()
        self.create = patch.object(release, "create_bump", return_value="bump").start()
        self.verify = patch.object(release, "verify_bump").start()
        self.pr = patch.object(release, "ensure_pull_request", return_value="https://github.com/owner/repo/pull/2").start()
        self.github.create_ref.side_effect = lambda name, sha: self.refs.update({name: ref(sha)})
        self.addCleanup(patch.stopall)
    ...
    def test_inspects_exact_main_and_skips_advanced_version(self):
        self.github.contents.return_value = manifest("1.2.4-dev.0")
        self.assertIsNone(self.run_post())
        self.github.contents.assert_called_once_with("Cargo.toml", "main")
    ...
    def test_creates_signed_bump_then_verifies_before_pr(self):
        ...
        self.clean.assert_called_once_with("main")
        self.github.create_ref.assert_called_once_with("heads/post-release/v1.2.3", "main")
        self.create.assert_called_once()
        self.verify.assert_called_once_with(self.github, "bump", "main", "1.2.3", "1.2.4-dev.0", self.changes.return_value)
    ...
    def test_recovers_empty_branch_when_main_advanced(self):
        ...
        self.github.api.side_effect = lambda path: (
            {"status": "ahead"} if path == "compare/old-main...main" else self.release_record
        )
        older_changes = {"Cargo.toml": manifest("1.2.4-dev.0")}
        with patch.object(release, "changes_at_parent", return_value=older_changes):
            self.run_post()
        self.github.create_ref.assert_not_called()
        self.create.assert_called_once_with(self.github, "post-release/v1.2.3", "old-main", "1.2.4-dev.0", older_changes)

    def test_main_advance_before_branch_creation_does_not_create_branch(self):
        self.github.ref.side_effect = [ref("tag-commit"), ref("main"), None, ref("new-main")]
        with self.assertRaisesRegex(release.ReleaseError, "advanced"):
            self.run_post()
        self.github.create_ref.assert_not_called()
        self.create.assert_not_called()
...
class CommitVerificationTests(unittest.TestCase):
    def setUp(self):
        self.github = Mock(repository="owner/repo")
        self.commit = {"verification": {"verified": True}, "message": "Start 1.2.4-dev.0 development", "parents": [{"sha": "main"}]}
        self.comparison = {"total_commits": 1, "files": [{"filename": "Cargo.toml", "status": "modified"}]}
        self.github.api.side_effect = lambda path: self.commit if path == "git/commits/bump" else self.comparison
        self.contents = manifest("1.2.4-dev.0")
        self.github.contents.return_value = self.contents

    def verify(self):
        release.verify_bump(self.github, "bump", "main", "1.2.3", "1.2.4-dev.0", {"Cargo.toml": self.contents})
    ...
    def test_rejects_added_unrelated_file(self):
        self.comparison["files"].append({"filename": "unrelated.txt", "status": "added"})
    ...
    def test_recovers_older_parent_only_when_on_main_history(self):
        ...
        self.github.api.side_effect = lambda path: (
            self.commit if path == "git/commits/bump" else
            {"status": "ahead"} if path == "compare/old-main...main" else self.comparison
        )
        with patch.object(release, "changes_at_parent", return_value={"Cargo.toml": self.contents}) as reconstruct:
            self.verify()
            reconstruct.assert_called_once_with(self.github, "old-main", "1.2.3", "1.2.4-dev.0")


class ApiAndPullRequestTests(unittest.TestCase):
    def test_clean_checkout_rejects_stale_head_and_untracked_work(self):
        with patch.object(release, "command", return_value="stale\n"):
            with self.assertRaisesRegex(release.ReleaseError, "stale"):
                release.clean_checkout("main")
        with patch.object(release, "command", side_effect=["main\n", "?? personal.txt\n"]):
    ...
    def test_create_ref_recovers_lost_response_only_for_matching_target(self):
        ...
        with patch.object(github, "api", side_effect=release.ReleaseError("lost response")), patch.object(github, "ref", return_value=ref("wanted")):
            github.create_ref("tags/v1.2.3", "wanted")
        with patch.object(github, "api", side_effect=release.ReleaseError("conflict")), patch.object(github, "ref", return_value=ref("other")):
            with self.assertRaises(release.ReleaseError):
                github.create_ref("tags/v1.2.3", "wanted")

    def test_permission_failure_is_not_missing_ref(self):
        github = release.GitHub("owner/repo")
        with patch.object(subprocess, "run", return_value=subprocess.CompletedProcess([], 1, "", "gh: Forbidden (HTTP 403)")):
            with self.assertRaises(release.ReleaseError):
    ...
    def test_graphql_uses_expected_head_and_inline_encoded_file_payload(self):
        ...
        with patch.object(release, "command", return_value=json.dumps({"data": {"createCommitOnBranch": {"commit": {"oid": "signed"}}}})) as run:
            self.assertEqual(release.create_bump(github, "post-release/v1.2.3", "main", "1.2.4-dev.0", {"Cargo.toml": b"content"}), "signed")
        payload = json.loads(run.call_args.kwargs["input_text"])["variables"]["input"]
        self.assertEqual(payload["expectedHeadOid"], "main")
        self.assertEqual(base64.b64decode(payload["fileChanges"]["additions"][0]["contents"]), b"content")

    def test_existing_open_pr_is_reused(self):
        github = Mock(repository="owner/repo")
        github.api.return_value = [{"number": 2, "state": "open", "head": {"sha": "bump"}, "labels": [{"name": "enqueue"}], "html_url": "https://github.com/owner/repo/pull/2"}]
        with patch.object(release, "command") as run:
            self.assertEqual(release.ensure_pull_request(github, "post-release/v1.2.3", "bump", "1.2.4-dev.0", "Body"), "https://github.com/owner/repo/pull/2")
        run.assert_not_called()

    def test_existing_pr_recovers_missing_enqueue_label(self):
        github = Mock(repository="owner/repo")
        github.api.return_value = [{"number": 2, "state": "open", "head": {"sha": "bump"}, "labels": [], "html_url": "https://github.com/owner/repo/pull/2"}]
        with patch.object(release, "command") as run:
            release.ensure_pull_request(github, "post-release/v1.2.3", "bump", "1.2.4-dev.0", "Body")
        run.assert_not_called()
        self.assertEqual(github.api.call_args.args, ("issues/2/labels",))
        self.assertEqual(github.api.call_args.kwargs, {"method": "POST", "data": {"labels": ["enqueue"]}})
    ...
    def test_new_pr_uses_body_file_and_enqueue_label(self):
        ...
        with patch.object(release, "command", side_effect=create):
            release.ensure_pull_request(github, "post-release/v1.2.3", "bump", "1.2.4-dev.0", "Exact body\n")
        self.assertEqual(observed_body, ["Exact body\n"])

    def test_pr_body_preserves_existing_template_sections(self):
        with release.temporary_directory() as directory, patch.object(release, "ROOT", Path(directory)):
            root = Path(directory)
            (root / "pull_request_template.md").write_text("## Summary\n\n## Validation\n")
            with self.assertRaisesRegex(release.ReleaseError, "template"):
                release.pull_request_body("1.2.3", "1.2.4-dev.0", None)
            body = root / "body.md"
            body.write_text("## Summary\n\nDevelopment bump.\n\n## Validation\n\nVersion check passed.\n")
            self.assertEqual(release.pull_request_body("1.2.3", "1.2.4-dev.0", body), body.read_text())
            body.write_text("## Unrelated section\n\nBody\n")
            with self.assertRaisesRegex(release.ReleaseError, "headings"):
                release.pull_request_body("1.2.3", "1.2.4-dev.0", body)

    def test_closed_pr_is_not_deleted_or_recreated(self):
        github = Mock(repository="owner/repo")
        github.api.return_value = [{"state": "closed", "html_url": "https://github.com/owner/repo/pull/2"}]
        with patch.object(release, "command") as run:
            with self.assertRaisesRegex(release.ReleaseError, "closed"):
                release.ensure_pull_request(github, "post-release/v1.2.3", "bump", "1.2.4-dev.0", "Body")
        run.assert_not_called()
...
class VersionRecoveryIntegrationTests(unittest.TestCase):
    def test_reconstructs_real_version_file_bump_from_api_contents(self):
        ...
        originals = {path.relative_to(ROOT).as_posix(): content.encode() for path, content in planned.items()}
        crate_paths = [f"crates/{name}/Cargo.toml" for name in tool.CRATES]
        originals.update({path: (ROOT / path).read_bytes() for path in crate_paths})
        github = Mock(repository="owner/repo")
        github.contents.side_effect = lambda path, sha: originals[path]
        with patch.object(release, "command", return_value="\0".join(crate_paths) + "\0"):
            changes = release.changes_at_parent(github, "old-main", "1.2.3", "1.2.4-dev.0")
        self.assertEqual(release.manifest_version(changes["Cargo.toml"]), "1.2.4-dev.0")
        self.assertIn("Cargo.lock", changes)
        self.assertEqual(json.loads(changes["npm/openapi-mcp-rs/package-lock.json"])["packages"][""]["version"], "1.2.4-dev.0")
        self.assertTrue(set(changes) <= {path.relative_to(ROOT).as_posix() for path in tool.version_files(ROOT)})
